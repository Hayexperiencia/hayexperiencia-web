  ...
  return (
    <div className="sticky top-24 space-y-4">
      <div className="p-6 rounded-2xl border border-[var(--color-border)] bg-white">
        <h3 className="text-lg font-semibold text-[var(--color-primary)] mb-1">{projectName}</h3>
        <p className="text-sm text-[var(--color-text-light)] mb-4">{location}</p>

        {price && (
          <div className="mb-4 p-3 rounded-xl bg-[var(--color-accent)]/10">
            <p className="text-xs text-[var(--color-text-light)]">Desde</p>
        ...
        {highlights && highlights.length > 0 && (
          <ul className="mb-4 space-y-2">
            {highlights.map((h) => (
              <li key={h} className="flex items-center gap-2 text-sm text-[var(--color-text-light)]">
                <svg className="h-4 w-4 text-[var(--color-accent)] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
