...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const existe = await p.query(`SELECT to_regclass('analytics.ga4_snapshot') IS NOT NULL AS x`);
    if (!existe.rows[0].x) {
      return jsonWithCache({ ok: true, pendiente: true, payload: null, capturedAt: null });
    }
    const r = await p.query(
      `SELECT payload, captured_at FROM analytics.ga4_snapshot ORDER BY captured_at DESC LIMIT 1`
    );
    if (r.rows.length === 0) {
      return jsonWithCache({ ok: true, pendiente: true, payload: null, capturedAt: null });
    }
